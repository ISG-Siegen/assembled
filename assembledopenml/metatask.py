...
class MetaTask:
    ...
    def filter_predictors(self, remove_bad_predictors: bool = True, remove_constant_predictors: bool = False,
        ...
        if remove_worse_than_random_predictors:
            ...
            random_performance = score_metric(self.ground_truth, random_predictions, pd.DataFrame(), self.class_labels,
                                              use_binarized_y_pred=True)
            ...
            pred_performance = self.predictions_and_confidences[self.predictors].apply(lambda x: score_metric(
                self.ground_truth, x, pd.DataFrame(), self.class_labels, use_binarized_y_pred=True), axis=0)
    ...
    def _save_fold_results(y_true, y_pred, fold_idx, out_path, technique_name, classification=True):
        ...
        if classification:
            ...
            input_dtype = {technique_name: "string", "ground_truth": "string"}
            res_df = res_df.astype(input_dtype)
        else:
            input_dtype = None
        ...
        if fold_idx == 0:
            if path_exists:
                # Load old data
                tmp_df = pd.read_csv(out_path, dtype=input_dtype).set_index(["ground_truth", "Index-Metatask", "Fold"])
    ...
    def run_ensemble_on_all_folds(self, technique, technique_args: dict, technique_name,
                                  ...
                                  pre_fit_base_models: bool = False, base_models_with_names: bool = True,
                                  label_encoder=True, fit_technique_on_original_data=False,
                                  preprocessor=None, output_file_path=None):
        ...
        base_models_with_names: bool, default=True
        ...
        label_encoder: bool, default=True
