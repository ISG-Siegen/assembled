...
from experiments.data_utils import get_id_and_validate_existing_data
import json
import os
...
def get_vba_sba_scores(meta_task, met):
    ...
    for idx, _, test_metadata in meta_task.fold_split(return_fold_index=True):
        ...
        # -- Gap related values
        vba_perf_list.append(met(i_y_test, o_y_pred_vba, pd.DataFrame(), i_y_test.unique().tolist(),
                                 use_binarized_y_pred=True))
        sba_perf_list.append(met(i_y_test, o_y_pred_sba, pd.DataFrame(), i_y_test.unique().tolist(),
                                 use_binarized_y_pred=True))
...
if __name__ == "__main__":
    ...
    benchmark_valid_tasks = {
        ...
        "benchmark_search_parameters": {
            ...
            "score_metric": score_metric.__name__,
